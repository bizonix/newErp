<?php
    ...
    $log	=	"\n\n\nDate: ".date("Y-m-d H:i:s"). " 开始速卖通(".$aliexpress_user.")的订单评价\n";
    @file_put_contents($logfile, $log, FILE_APPEND);
    ...
    while(1){
    	...
    	if ( !$dataList['success'] || $dataList['totalItem'] == 0) {
    		$log = "\n\n\nDate: ".date("Y-m-d H:i:s"). "账号".$aliexpress_user."的待评价订单拉取完成\n";
    		@file_put_contents($logfile, $log, FILE_APPEND);
    		break;
    	}
    ...
    foreach ($orderList as $order) {
    	...
    	if ($result['success']) {
    		$log = "\n\n\nDate: ".date("Y-m-d H:i:s"). "订单".$orderId."评价成功\n";
    		@file_put_contents($logfile, $log, FILE_APPEND);
    		//echo "---账号--$aliexpress_user-----订单--$orderId--------评价成功!\n";
    	} else {
    		$log = "\n\n\nDate: ".date("Y-m-d H:i:s"). "订单".$orderId."评价失败，错误信息:".$result['errorMessage']."\n";
    		@file_put_contents($logfile, $log, FILE_APPEND);
