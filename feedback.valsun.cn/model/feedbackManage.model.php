...
*iqc检测领取 
*/
class FeedbackManageModel{	
	public 	static $dbConn;
	public	static $errCode	=	0;
	public	static $errMsg	=	"";
	static  $table			=	"fb_comment_order_record";
	...
	public 	static function getOrderList($select,$where){
		self::initDB();
		$sql	 =	"select {$select} from ".self::$table." as a left join fb_account as b on a.sellerAccountId = b.id  {$where} ";
		//echo $sql;
	...
	public 	static function getOrderNum($where){	
		self::initDB();
		$sql	 =	"select a.id from ".self::$table." as a left join fb_account as b on a.sellerAccountId = b.id  {$where} ";	
		$query	 =	self::$dbConn->query($sql);
		if($query){
	...
	//goods_code转换为sku
	public static function goods_codeTosku($goods_code){
		self::initDB();
		$sql   = "SELECT sku FROM `qc_sample_info` WHERE skuCode='{$goods_code}'";
		$query = self::$dbConn->query($sql);
		if($query){                        
