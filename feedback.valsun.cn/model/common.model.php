...
class CommonModel{	
	...
	//获取条件料号列表
	public 	static function getGoodsInfo($select,$where){
		self::initDB();
	...
	//获取条件料号列表
	public 	static function getGoodsCombineInfo($select,$where){
		self::initDB();
	...
	//获取条件料号列表
	public 	static function getCombineSkuInfo($select,$where){
		self::initDB();
		$sql	 =	"select {$select} from pc_sku_combine_relation {$where} ";
		//echo $sql;
		$query	 =	self::$dbConn->query($sql);
		if($query){
			$ret =self::$dbConn->fetch_array_all($query);
	...
	//获取条件料号列表
	public 	static function judgeCombineSku($sku){
		self::initDB();
		$sql	 =	"select count(*) from pc_goods_combine where combineSku = '{$sku}'";		
		$query	 =	self::$dbConn->query($sql);
		if($query){
			$ret =self::$dbConn->fetch_array_all($query);			
	...
	//获取条件料号列表
	public 	static function getPurchaserInfo($select,$where){
		self::initDB();
		$sql	 =	"select {$select} from power_global_user {$where} ";
		//echo $sql;
		$query	 =	self::$dbConn->query($sql);		
		if($query){
			$ret =self::$dbConn->fetch_array_all($query);
