...
//系数名称联动
function change_cNameSkip(){
	//alert("aaaaaaaaaa");return false;
	var sampleTypeOnId = $("#sampleTypeOnId").val();
	if(sampleTypeOnId==0){
		$('#cNameSkip').html('');
		var newtab = '';
		newtab +="<label for='cNameOn'>系数:</label>";
		newtab +="<select  name='cNameOn' id='cNameOn'>";
		newtab +="<option value='0'>请选择</option>";
		newtab +="</select>";
		$("#cNameSkip").html(newtab);
		return false;
	}
	$.ajax({
			type	: "POST",
			dataType: "jsonp",
			url		: 'json.php?mod=sampleCoefficient&act=getCnameInfo&jsonp=1',
			data	: {id:sampleTypeOnId},
			success	: function (msg){
				if(msg.errCode==0){
					...
					newtab +="<option value='0'>请选择</option>";
					for(var i=0;i<len;i++){
						newtab +="<option value='"+msg.data[i].cName+"'>"+msg.data[i].cName+"</option>";
					}
					newtab +="</select>";
					$("#cNameSkip").html(newtab);
				}else{
