...
*作者：hws
*/
class WhAreaAct extends Auth{
	static $errCode	=	0;
	static $errMsg	=	"";
	
	//获取仓位信息
	function  act_getAreaList($select = '*',$where){
		$list =	WhAreaModel::getAreaList($select,$where);
		if($list){
			return $list;
		}else{
			self::$errCode = WhAreaModel::$errCode;
			self::$errMsg  = WhAreaModel::$errMsg;
			return array();
		}
	}
	
	//获取仓位信息
	function  act_selectAreaList($select = '*',$where){
		$areaName = $_POST['areaname'];
		$storeId = $_POST['storeId'];
		$floorId = $_POST['floorId'];
		
		$list =	WhAreaModel::getAreaList("*","where areaName='$areaName' and storeId='$storeId' and floorId='$floorId' and is_delete=0 ");
		self::$errCode = WhAreaModel::$errCode;
		self::$errMsg  = WhAreaModel::$errMsg;
		if($list){
			return $list[0];
		...
		foreach($position_a as $po_a){
			...
			if(!empty($pname)){
				...
				if(!empty($lists)){
					...
					$list 		= PositionModel::getPositionList("*","where x_alixs='$x_alixs' and y_alixs='$y_alixs' and z_alixs='$position_d[1]' and floor='$floor' and storeId={$storeid}");
					if(!empty($list)){
						$data = array(
							'pName'     => $position_d[0],
							'is_enable' => $position_d[2],
	...
	public function  act_addArea(){
		...
		$rt = $this->act_getAreaList('*'," where areaName = '".$areaName."' and v_x_alixs = '".$axis_x."' and v_y_alixs = '".$axis_y."' and storeId = '".$storeId."' and floorId = '".$floorId."' and is_delete = 0 ");
		if($rt){
			self::$errCode = '100002';
			self::$errMsg  = '仓位名称不变';
			return false;	
		...
		$rt = $this->act_getAreaList('*'," where areaName = '".$areaName."' and storeId = '".$storeId."' and floorId = '".$floorId."' and is_delete = 0 ");
		if($rt){
			self::$errCode = '100001';
			self::$errMsg  = '不允许同一楼层出现相同区域名称';
			return false;	
		}
		
		$rt = $this->act_getAreaList('*'," where v_x_alixs = '".$axis_x."' and v_y_alixs = '".$axis_y."' and storeId = '".$storeId."' and floorId = '".$floorId."' and is_delete = 0 ");
		if($rt){
			
			$rt = $this->act_updateAreaList(array('areaName'=>$areaName), " where v_x_alixs = '".$axis_x."' and v_y_alixs = '".$axis_y."' and storeId = '".$storeId."' and floorId = '".$floorId."' and is_delete = 0 ");
			return $rt;
		}
		
		$data = array();
		$data['v_x_alixs'] = $axis_x;
		$data['v_y_alixs'] = $axis_y;
		$data['storeId'] = $storeId;
		$data['floorId'] = $floorId;
		$data['areaName'] = $areaName;
		
		$list =	WhAreaModel::addArea($data);
		self::$errCode = WhAreaModel::$errCode;
		self::$errMsg  = WhAreaModel::$errMsg;
		return $list;
	}
	...
	//更新区域名称
	public function act_updateAreaList($data, $where){
		$rt =	WhAreaModel::updateAreaList($data, $where);
		self::$errCode = WhAreaModel::$errCode;
		self::$errMsg  = WhAreaModel::$errMsg;
		return $rt;	
	}
