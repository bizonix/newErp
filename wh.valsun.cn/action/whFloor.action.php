...
*add by :herman.xi
*/
class WhFloorAct extends Auth{
	static $errCode	=	0;
	static $errMsg	=	"";	
	
	//获取信息
	function  act_getFloorList($select = '*',$where){
		$list =	M('WhFloor')->getFloorList($select,$where);
		if($list){
			return $list;
		}else{
	...
	//获取当前仓位信息
	function  act_addWhFloor(){
		$info 	 = array();
		$floors_DC = array(1=>'一',2=>'二',3=>'三',4=>'四',5=>'五',6=>'六',7=>'七',8=>'八',9=>'九',10=>'十',11=>'十一',12=>'十二',13=>'十三',14=>'十四',15=>'十五',16=>'十六',17=>'十七',18=>'十八',19=>'十九',20=>'二十');
		$whFloorName_val 	= $floors_DC[$_POST['whFloorName_val']].'楼';
		$whFloorCode_val 	= $_POST['whFloorName_val'];
		$storeId   			= $_POST['storeId'];
		$info['whFloor'] 	= $whFloorName_val;
		$info['whCode'] 	= $whFloorCode_val;
		$info['storeId'] 	= $storeId;
		$rt    				= WhFloorModel::addFloor($info);
	
		self::$errCode 		= WhFloorModel::$errCode;
		self::$errMsg  		= WhFloorModel::$errMsg;
		return $rt;
