...
function check_ebay_sku(){
	...
	var now_sku_obj=document.getElementById('now_sku');
	var now_sku	= now_sku_obj.value;
	var now_pname_obj=document.getElementById('now_pname');
	var now_pname = now_pname_obj.value;
	var now_detailid_obj=document.getElementById('now_detailid');
	var now_detailid = now_detailid_obj.value;
	var ebay_sku_obj=document.getElementById('ebay_sku');
	//var ebay_sku = ebay_sku_obj.value.toUpperCase();
	var ebay_sku = ebay_sku_obj.value;
	var keyCode = event.keyCode;
	if (keyCode!=13) return false;
	//$("showTime").innerHTML = "";
	if(ebay_sku != ''){
		scanProcessTip('扫描料号...',true);
		scanProcess2(ebay_id, ebay_sku, now_sku, now_pname ,now_detailid);
	}else{
		scanProcessTip('料号录入失败,请重试',false);
...
function check_ebay_sku2(){
	...
	if(pskunum == ''){
		...
		scanProcessTip('扫描料号...',true);
		scanProcess21(ebay_id, ebay_sku, pskunum, now_sku, now_pname, now_detailid);
...
function scanProcess2(ebay_id, ebay_sku, now_sku, now_pname, now_detailid){
	createXMLHttpRequest();
	var url = 'json.php?mod=scanAllSkuOrder&act=checkSku&jsonp=1';
	var param = 'ebay_id='+ebay_id+'&ebay_sku='+ebay_sku+'&now_sku='+now_sku+'&now_pname='+now_pname+'&now_detailid='+now_detailid;
	xmlHttpRequest.open("POST",url,true);
	xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded"); 
...
function scanProcess21(ebay_id, ebay_sku, pskunum, now_sku, now_pname, now_detailid){
	createXMLHttpRequest();
	var url = 'json.php?mod=scanAllSkuOrder&act=checkSku&jsonp=1';
	var param = 'ebay_id='+ebay_id+'&ebay_sku='+ebay_sku+'&pskunum='+pskunum+'&now_sku='+now_sku+'&now_pname='+now_pname+'&now_detailid='+now_detailid;
	xmlHttpRequest.open("POST",url,true);
	xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded"); 
...
function scanProcess3(ebay_id,sku,now_pname){
	createXMLHttpRequest();
	var url = 'json.php?mod=scanAllSkuOrder&act=searchSku&jsonp=1';
	var param = 'ebay_id='+ebay_id+'&sku='+sku+'&now_pname='+now_pname;
	xmlHttpRequest.open("POST",url,true);
	xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded"); 
		...
		if(xmlHttpRequest.status == 200){  
			...
			if( data.errCode == 0 ){//扫描处理完毕一个包裹,进入下一个包裹处理流程
				...
				if(show_list(data.data)){
					document.getElementById('now_sku').value = data.data.detail[0].sku;
					document.getElementById('now_pname').value = data.data.detail[0].gl;
					document.getElementById('now_detailid').value = data.data.detail[0].detailId;
					show_scansku();
	...
	if(xmlHttpRequest.readyState == 4){  
		if(xmlHttpRequest.status == 200){
			...
			if(data.errCode == '300'){//扫描处理完毕一个包裹,进入下一个包裹处理流程
				...
				scanProcessTip(data.errMsg,true);
				//setTimeout(function(){
					lhref = 'index.php?mod=pda_orderPicking&act=pda_orderPicking';
...
function show_list(datas){
	...
	newtable += '<tr style="font-size:13px" align="center"><td>仓位号</td><td align="center">SKU</td><td align="center">数量</td><td align="center">库存</td></tr>';
	for(var i=0; i<datas.detail.length; i++){
		newtable += '<tr style="font-size:13px" align="center"><td  width="55px" style="padding:2px">'+datas.detail[i].gl+'</td>';
		newtable += '<td  width="80px">'+datas.detail[i].sku+'</td>';
		newtable += '<td width="35px">'+datas.detail[i].nums+'</td>';
		newtable += '<td width="35px">'+datas.detail[i].goods_count+'</td></tr>';
		break;
	}
	/*
...
function inquire_sku(sku){
	...
	var now_pname	= $('now_pname').value;
	var keyCode = event.keyCode;
	if (keyCode!=13) return false;
	if(sku != ''){
		scanProcess3(ebay_id,sku,now_pname);
	}else{
