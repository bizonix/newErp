...
function scanProcess(orderId){
	...
	var url = 'json.php?mod=pda_orderInCheckB&act=getOrderInfo&jsonp=1';
...
function scanProcess1(orderId,sku){
	createXMLHttpRequest();
	var url = 'json.php?mod=pda_orderInCheckB&act=checkSku&jsonp=1';
	...
	if(xmlHttpRequest.readyState == 4){  
		if(xmlHttpRequest.status == 200){
			...
			if( data.errCode == 0){
				...
				newtable += '</table>';
				newtable += '<span style="font-size:15px; padding-top:2px;">本次接收数量:</span><input type="text" class="textinput" name="sku_num" id="sku_num" onkeydown="checkSkuNum()" value="" />';
...
function scanProcess2(orderId, sku, sku_num){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_orderInCheckB&act=checkSkuNum&jsonp=1';
...
function header_page(cid){
	if(cid != undefined){
		switch(cid){
			case 15:
				//配货清单出库
				lhref = 'index.php?mod=pda_orderInCheckB&act=pda_scanPickList';
				break;
		}
...
function scanProcess3(orderId,now_orderId,now_sku,now_pname){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_orderInCheckB&act=nextSku&jsonp=1';
...
function scanProcess4(group, now_orderId){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_orderInCheckB&act=endAssignList&jsonp=1';
