...
function scanProcess(order_group){
	...
	var url = 'json.php?mod=pda_SkuOutCheck&act=getGroupInfo&jsonp=1';
...
function scanProcess1(order_group,sku){
	createXMLHttpRequest();
	var url = 'json.php?mod=pda_SkuOutCheck&act=checkSku&jsonp=1';
	...
	if(xmlHttpRequest.readyState == 4){  
		if(xmlHttpRequest.status == 200){
			...
			if( data.errCode == 0){
				...
				newtable += '<table id="sku_tab" cellpadding="0" cellspacing="0" border="1" width="100%">';
				newtable += '<tr style="font-size:13px"><td style="padding:2px">SKU</td><td>配货数</td><td>已复核</td></tr>';
...
function scanProcess2(order_group, sku, sku_num, now_group_id){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_SkuOutCheck&act=checkSkuNum&jsonp=1';
...
function header_page(cid){
	if(cid != undefined){
		switch(cid){
			case 15:
				//配货清单出库
				lhref = 'index.php?mod=pda_pda_SkuOutCheck&act=pda_scanPickList';
				break;
		}
...
function scanProcess3(order_group,now_group_id,now_sku,now_pname){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_SkuOutCheck&act=nextSku&jsonp=1';
...
function scanProcess4(order_group,sku,now_sku,submit_orders,submit_nums,now_group_id,now_pname){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_SkuOutCheck&act=submitInfo&jsonp=1';
