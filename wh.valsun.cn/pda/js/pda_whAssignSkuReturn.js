...
function scanProcess(order_group){
	...
	var url = 'json.php?mod=pda_whAssignSkuReturn&act=getGroupInfo&jsonp=1';
...
function scanProcess1(order_group,sku){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_whAssignSkuReturn&act=checkSku&jsonp=1';
		...
		if(xmlHttpRequest.status == 200){
			...
			if( data.errCode == 0){
				...
				*/
				...
				//if(now_sku_num==1){
//					newtable += '<span style="font-size:15px">退库数量:</span><input type="text" class="textinput" name="sku_num" id="sku_num" onkeydown="checkSkuNum()" value="'+now_sku_num+'" />';
//					showdetail.innerHTML = newtable;
//					checkSkuNum2();
//				}else{
					newtable += '<span style="font-size:15px">退库数量:</span><input type="text" class="textinput" name="sku_num" id="sku_num" onkeydown="checkSkuNum()" value="" />';
					showdetail.innerHTML = newtable;
...
function checkSkuNum(){
	...
	if (keyCode == 13) {
		...
		if(sku_num==''){
			scanProcessTip('退库数量不能为空，请确认!',false);
			return false;
		}
		sku_num_obj.blur();
		//scanProcessTip('数据正在提交，请稍等!',true);
...
function scanProcess2(order_group,sku,sku_num,now_group_id){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_whAssignSkuReturn&act=checkSkuNum&jsonp=1';
...
function scanProcessResponse2(){
	...
	if(xmlHttpRequest.readyState == 4){  
		if(xmlHttpRequest.status == 200){
			...
			if( data.errCode == 0){
			   scanProcessTip(data.errMsg,true);
				var order_group = document.getElementById('group_id');
                order_group.focus();
                order_group.value='';
...
function header_page(cid){
	if(cid != undefined){
		switch(cid){
			case 15:
				//配货清单出库
				lhref = 'index.php?mod=pda_pda_whAssignSkuReturn&act=pda_scanPickList';
				break;
		}
...
function scanProcess3(order_group,now_group_id,now_sku,now_pname){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_whAssignSkuReturn&act=nextSku&jsonp=1';
...
function scanProcess4(order_group,sku,now_sku,submit_orders,submit_nums,now_group_id,now_pname){
	createXMLHttpRequest();  
	var url = 'json.php?mod=pda_whAssignSkuReturn&act=submitInfo&jsonp=1';
