...
function checksku(){
	...
	if(e.keyCode==13 || e.keyCode==10){
		...
		var param = 'sku='+sku+'&storeId='+storeId;
	...
	if(xmlHttpRequest.readyState == 4){  
		if(xmlHttpRequest.status == 200){  
			...
			if(data.errCode==0){
                ...
                document.getElementById('now_position_list').style.display = '';
                document.getElementById('now_store_list').style.display = '';
	...
	if(xmlHttpRequest.readyState == 4){  
		if(xmlHttpRequest.status == 200){  
			...
			if(data.errCode==0){
                ...
                document.getElementById('now_position_list').style.display = 'none';
                document.getElementById('now_store_list').style.display = 'none';
...
function checknums(){
	...
	if(e.keyCode==13 || e.keyCode==10){
		...
		if(confirm('上架入库数量为'+nums+',是否确认!')){
			...
			createXMLHttpRequest();
			var url = './json.php?act=whShelf&mod=whShelf&jsonp=1';
			var param = 'sku='+sku+"&nums="+nums+"&select_now_position="+select_now_position+"&select_now_store="+select_now_store+"&select_hope_position="+select_hope_position+"&select_hope_store="+select_hope_store+'&storeId='+storeId;
			xmlHttpRequest.open("POST",url,true); 
			xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded"); 
...
function show_list(datas){
	...
	var newtable = '';
	newtable += '<fieldset id="sku_tab_fieldset" style=" float:left; margin-top:2px; padding:3px 0;width:85%;"><legend>现存储仓位</legend>';
	...
	newtable += '<select name="select_now_position" id="select_now_position" style="width:100%;">';
	if(datas!=null){
...
function show_list1(datas){
	...
	showdetail.innerHTML = '';
	var newtable = '';
	newtable += '<fieldset id="sku_tab_fieldset" style=" float:left; margin-top:2px; padding:3px 0;width:85%;"><legend>现存货位置</legend>';
	newtable += '<table id="sku_tab" cellpadding="0" cellspacing="0" border="0" width="100%">';
	//newtable += '<tr><td>仓位号</td><td>数量</td></tr>';
	newtable += '<tr><td colspan="2">';
	newtable += '<select name="select_now_store" id="select_now_store" style="width:100%;" onchange="change_now_store();">';
	if(datas!=null){
...
function show_list2(datas){
	...
	showdetail.innerHTML = '';
	var newtable = '';
	newtable += '<fieldset id="sku_tab_fieldset" style=" float:left; margin-top:2px; padding:3px 0;width:width:85%;;"><legend>最近空仓位号</legend>';
	newtable += '<table id="sku_tab" cellpadding="0" cellspacing="0" border="0" width="100%">';
	//newtable += '<tr><td>仓位号</td></tr>';
	newtable += '<tr><td>';
	newtable += '<select name="select_hope_position" id="select_hope_position" style="width:100%;" onchange="change_hope_position();">';
	if(datas!=null){
...
function show_list3(datas){
	...
	showdetail.innerHTML = '';
	var newtable = '';
	newtable += '<fieldset id="sku_tab_fieldset" style=" float:left; margin-top:2px; padding:3px 0;width:width:85%;;"><legend>最近空存货位号</legend>';
	newtable += '<table id="sku_tab" cellpadding="0" cellspacing="0" border="0" width="100%">';
	//newtable += '<tr><td>仓位号</td></tr>';
	newtable += '<tr><td>';
	newtable += '<select name="select_hope_store" id="select_hope_store" style="width:100%;" onchange="change_hope_store();">';
	if(datas!=null){
		var now_position = document.getElementById("now_position").value.toUpperCase();
		newtable += '<option value="0">选择存货位置</option>';
		for(var i=0; i<datas.length; i++){
