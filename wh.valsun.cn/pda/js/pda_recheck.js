...
function getSkuList(e){
	...
	var url = 'json.php?mod=recheck&act=getSkuList&jsonp=1';
...
function rebuildskulist(list){
	...
	for(i in list){
        ...
        opt.innerHTML = '料号--'+list[i]['sku'] + ' 数量:--[' + list[i]['amount'] + ']';//alert(opt);
...
function scansubmit(e){
	...
	showOkMsg('正在复核...');
	createXMLHttpRequest();
	var url = 'json.php?mod=recheck&act=recheckInfoSubmit&jsonp=1';
...
function signUnusual(){
	var shipOrderId = document.getElementById('orderidinput').value;
	if(shipOrderId == ''){
		showErrorMsg('发货单号不能为空');
	}
	createXMLHttpRequest();
	var url = 'json.php?mod=recheck&act=signUnusual&jsonp=1';
	var param = 'shipOrderId=' + shipOrderId;
	
	xmlHttpRequest.open("POST",url,true);
	xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	xmlHttpRequest.onreadystatechange = signUnusualResponse;
	xmlHttpRequest.send(param);
}

function signUnusualResponse(){
	if(xmlHttpRequest.readyState == 4){
		if(xmlHttpRequest.status == 200){
			var res = xmlHttpRequest.responseText;
			var res=eval('('+res+')');
