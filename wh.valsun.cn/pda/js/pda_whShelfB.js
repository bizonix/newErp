...
function checksku(){
	...
	if(e.keyCode==13 || e.keyCode==10){
		...
		var url = './json.php?act=whShelfSku&mod=whShelfB&jsonp=1';
		var param = 'sku='+sku;
...
function checkposition(){
	...
	if(e.keyCode==13 || e.keyCode==10){
		...
		createXMLHttpRequest();
		var url = './json.php?act=findPosition&mod=whShelfB&jsonp=1';
...
function checknums(){
	...
	if(e.keyCode==13 || e.keyCode==10){
		...
		if(confirm('上架入库数量为'+nums+',是否确认!')){
			...
			createXMLHttpRequest();
			var url = './json.php?act=whShelf&mod=whShelfB&jsonp=1';
