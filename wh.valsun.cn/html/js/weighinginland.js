...
function getSkuList(e){
    ...
    showOkMsg('验证订单id...');
    $.getJSON('json.php?mod=weighingScan&act=isOrderValideFZ&jsonp=1&orderid=' + orderid, function(data){
...
function scansubmit(e){
	...
	var expresstr = $('#expressinput').val();
	expresstr = $.trim(expresstr);
	if(expresstr.length == 0){
		showErrorMsg('请输入快递单号');
		focusInput("expressinput");
		return;
	}
    ...
    $.ajax(
			{
				type : "post",
				dataType:'json',
				url : 'json.php?mod=weighingScan&act=weighingSubmitFz&jsonp=1',
				data : {'orderid':orderid, 'num':num, 'userid':username, 'express':expresstr},
...
function getWeight(){
	var weight = window.document.app.currentweight;
	$("#skunumberinput").val(weight);
}
