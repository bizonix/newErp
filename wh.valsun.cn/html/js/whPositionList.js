$(function(){
	...
	$('#InitPartition').click(function(){
		$('#mess').html('更新中请稍等...');
		var areaId = $('#areaId').val();
		//alert(areaId); return false;
		$.ajax({
			type    : "POST",
			dataType: "jsonp",
			url     : "json.php?mod=WhPosition&act=InitPartition&jsonp=1",
			data    : {"areaId":areaId},
			success	: function (msg){
				//console.log(msg);return false;
				if(msg.errCode==0){
	...
	$('.position').click(function(){
		...
		$.ajax({
			type    : "POST",
			dataType: "jsonp",
			url     : "json.php?mod=WhPosition&act=getCurrentPosition&jsonp=1",
			data	: {axis_x:axis_x,axis_y:axis_y,floor:floor,areaId:areaId,storey:storey,storeid:storeid},
			success	: function (msg){
				...
				if(msg.errCode==0){
					...
					if( (type1 && type1==1 )|| !name1){type1 = "checked";}
					...
					if((type2 && type2==1)|| !name2){type2 = "checked";}
					...
					if( (type3 && type3==1) || !name3){type3 = "checked";}
					...
					if((type4 && type4==1) || !name4){type4 = "checked";}
					...
					if((type5 && type5==1) || !name5){type5 = "checked";}
					...
					if((type6 && type6==1) || !name6){type6 = "checked";}
					...
					if((type7 && type7==1) || !name7){type7 = "checked";}
					...
					if((type8 && type8==1) || !name8){type8 = "checked";}
					...
					if( (type9 && type9==1) || !name9){type9 = "checked";}
					...
					+'<tr><td colspan="3" ><button align="bottom" axis_x="'+axis_x+'" axis_y="'+axis_y+'" floor="'+floor+'" class="adds" storeid="'+storeid+'" storey="'+storey+'" areaId="'+areaId+'" coordinate="'+coordinate+'" value="确定">确定</button> <button class="closes" value="取消">取消</button><td></tr>'
					+'</table>';
	...
	$('.adds').live('click', function(){
		...
		var first_name = name1.split(',');
        first_name     = first_name[0];
		$.ajax({
			type    : "POST",
			dataType: "jsonp",
			url     : "json.php?mod=WhPosition&act=opisitionManage&jsonp=1",
			data	: {position:position_str,axis_x:axis_x,axis_y:axis_y,floor:floor,storeid:storeid,storey:storey,areaId:areaId},
			success	: function (msg){
				//console.log(msg);return false;
				if(msg.errCode==0){
					hideTip();
					$('#'+id_obj).html(first_name);
				}else{
					alert(msg.errMsg);
				}				
			}
...
function update_position_route(){
    alertify.confirm('该操作会重置所有仓位索引，是否继续？',function(e){
        if(e){
            tips	= "<span id='label-tips' style='line-height:180%;font-size:14px;display:block;height:200px;'></span>";
        	alertify.alert(tips);
        	$("#alertify-ok").hide();
        	$("#label-tips").html("正在生成仓位索引,请稍候...<br/>处理期间，请不要关闭或刷新当前页面，谢谢配合！");
            $.getJSON('/json.php?mod=makeRouteIndex&act=makePositionIndex&jsonp=1', function(data){
