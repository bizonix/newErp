...
function header_page(cid){
	if(cid != undefined){
		switch(cid){
			case 15:
				//配货清单出库
				lhref = 'index.php?mod=ScanPdaPickList&act=scanPickList';
				break;
		}
...
function next_sku(){
	...
	var order_group	= group_obj.value;
	scanProcessTip('数据正在提交，请稍等!',true);
	scanProcess3(order_group,now_group_id,now_sku);	
}

function scanProcess3(order_group,now_group_id,now_sku){
	createXMLHttpRequest();  
	var url = 'json.php?mod=scanPdaPickList&act=nextSku&jsonp=1';
	var param = 'order_group='+order_group+'&now_group_id='+now_group_id+'&now_sku='+now_sku;
	xmlHttpRequest.open("POST",url,true); 
	xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
...
function submitInfo(){
	...
	if (keyCode == 13) {
		...
		//scanProcessTip('数据正在提交，请稍等!',true);
		scanProcess4(order_group,sku,now_sku,submit_orders,submit_nums,now_group_id);
	}
}

function scanProcess4(order_group,sku,now_sku,submit_orders,submit_nums,now_group_id){
	createXMLHttpRequest();  
	var url = 'json.php?mod=scanPdaPickList&act=submitInfo&jsonp=1';
	var param = 'order_group='+order_group+'&sku='+sku+'&now_sku='+now_sku+'&submit_orders='+submit_orders+'&submit_nums='+submit_nums+'&now_group_id='+now_group_id;
	xmlHttpRequest.open("POST",url,true); 
	xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
