...
class Auth{
	...
	private static $actionURL  = 'http://power.valsun.cn/api/json.php';//192.168.200.154正式环境
	//private static $actionURL  = 'http://dev.power.valsun.cn/api/json.php';//192.168.200.222开发环境
	private static $systemName = 'Warehouse';//接口系统名称  类型：string
	private static $token      = 'b5fa8197fd7a8727484e696ab547d031';//  类型：string
	...
	*参数为:用户名、密码、所属公司编号
	*/
	public static function login($user_name='',$pwd='',$version='V1.5')
	{   
		$user_name = self::strToHex($user_name);
		$pwd       = self::strToHex($pwd);
		$version   = self::strToHex($version);
		$params = 'mod=LoginAct&act=login&userName='.$user_name.'&pwd='.$pwd.'&version='.$version;  
		//echo $params.'<br/>';
