...
$(document).ready(function(){
	...
	$(function(){
		...
		url  = "http://purchase.valsun.cn/json.php?mod=common&act=getSkuImg&callback=?";
		skuObj	= $('.skuimg');
		...
		$.getJSON(url,{"skuArr":skuArr,"size":60},function(rtn){
			var imgObj = $.parseJSON(rtn).data;
			$.each(imgObj,function(i,item){
				$("#imgs-"+item.sku).attr("src",item.imgUrl);
				//$("img [data-sku="+item.sku+"]").attr("src",item.imgUrl);
				$("#imgb-"+item.sku).attr("href",item.sourceUrl);
				//$("img [data-sku=imgb-"+item.sku+"]").attr("href",item.sourceUrl);
			});
		});

/*
		$.getJSON(url,{"skuArr":skuArr,"size":100},function(rtn){
			var imgObj = $.parseJSON(rtn).data;
			$.each(imgObj,function(i,item){
				$("#imgb-"+item.sku).attr("href",item.imgUrl);
				$("[data-sku=imgb-"+item.sku+"]").attr("href",item.imgUrl);
			});
		});
		*/
	});
