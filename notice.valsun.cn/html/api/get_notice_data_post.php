...
$type_arr = explode(",", $_REQUEST['type']);
$flag	  = 0;
...
//验证数据返回格式
if(isset($_REQUEST['format']) && !in_array($_REQUEST['format'],array('xml','json'))) {
...
$losePerson = '';
if(isset($_REQUEST['losePerson']) && !empty($_REQUEST['losePerson'])) {
	$losePerson = trim($_REQUEST['losePerson']);
}

//获取客户端ip
$client_ip = '';
if(isset($_REQUEST['client_ip']) && !empty($_REQUEST['client_ip'])) {
	$client_ip = trim($_REQUEST['client_ip']);
}

//获取sysName
$sysName = '';
if(isset($_REQUEST['sysName']) && !empty($_REQUEST['sysName'])){
	$sysName = urldecode(trim($_REQUEST['sysName']));
}
...
switch($action) {
	case "send":											//发送消息
		...
		if(empty($ret) || empty($ret[0]['global_user_name'])) {										//获取发送者信息,发送人不存在直接退出
		...
		$to_arrs = array_unique($to_arrs);															//去掉重复的接收者

		foreach($to_arrs as $tonames) {																//获取并过滤接收者信息,查询数据库,去除有误的收件人 start
			$table 	= "`power_global_user` ";
			$filed 	= "global_user_email,global_user_phone,global_user_name,global_user_login_name";
			...
			if($to_login_name == $from_login_name) {												//对比是否是发送者本人
				if(empty($sysName)) {																//如果有系统参数
					$toSelf = true;
					continue;
				} else {
			...
			if(!empty($to_email) || !empty($to_mobile)) {											//存储接收者信息 至少有一个地址不为空
				$to_detail[] = array(
								'to_name'		=>	$to_name,
			...
			if(empty($to_mobile)) {																	//手机是否为空
				$noMobile[] = $to_name;
			}
		}																							//获取并过滤接收者信息 end

		if(count($to_detail) == 0) {																//无任何可用数据直接退出
		...
		if(in_array("sms", $type_arr)) {																	//发短信
			$table	= "nt_sms";
			foreach($to_detail as $to_detail_val) {															//单条发送消息start
		...
		if(in_array("sms", $type_arr) && !in_array("email", $type_arr)) {								//$status  短信000 或者邮件1 为成功
			$noEmail 	= array();
		} elseif(in_array("email", $type_arr) && !in_array("sms", $type_arr)) {
			$noMobile 	= array();
		}

		if($losePerson) {																				//收集发送失败人
			$errUser 			= array();
...
function sendEmail($title, $content, $email) {															//发送邮件函数
...
function newSendEmail($title, $content, $arrEmailName, $from) {
	...
	//echo "Mailer Error: " . $mail->ErrorInfo;
		...
		} elseif(strpos($err, 'data not accepted')!==false) {
