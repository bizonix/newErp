...
EOF;
    ...
    function view_getOderInfo(){
        ...
        $paramAr   = array('buyeraccount'=>$userid, 'selleraccount'=>$seller);//print_r($paramAr);exit;
//         $result = getOpenSysApi(OPENGETORDER, array('type'=>'orderinfo','buyeraccount'=>$userid, 'selleraccount'=>$seller));    //老系统接口
        $result_new = getOpenSysApi('order.getEbayShipOrderInfo', $paramAr);
//         print_r($result_new);exit;
...
EOF;
        ...
        if (!empty($result_new)) {
        	foreach ($result_new as $value) {
        	    $paytime        = isset($value['orderInfo']['paymentTime']) && !empty($value['orderInfo']['paymentTime'])   ? date('Y-m-d H:i:s',  $value['orderInfo']['paymentTime']):'';       //付款日期
        	    $shiptime       = isset($value['orderInfo']['scanTime']) && !empty($value['orderInfo']['scanTime'])   ? date('Y-m-d H:i:s', $value['orderInfo']['scanTime']):'';        //发货日期
        	    $couny          = isset($value['orderInfo']['currency']) && !empty($value['orderInfo']['currency'])         ? $value['orderInfo']['currency']:'';                       //币种
//         	    echo $value;exit;
        	    $address    = $value['orderInfo']['username'].',&nbsp;'.$value['orderInfo']['street'].'&nbsp;'.
                	    $value['orderInfo']['address2'].'&nbsp;'.$value['orderInfo']['city'].'&nbsp;'.$value['orderInfo']['state'].',&nbsp;'.
        	           $value['orderInfo']['zipCode'].',&nbsp;'.$value['orderInfo']['countryName'];
        	    if ($default_addr === FALSE) {
        	        $default_addr  = $address;
        	    }
        	    $address    = str_replace('"', '\"', $address);
        	    $buyer_account  = isset($value['orderInfo']['platformUsername'])? $value['orderInfo']['platformUsername']:''; //买家账号
        	    $money          = isset($value['orderInfo']['actualTotal'])     ? $value['orderInfo']['actualTotal']:'';      //金额
        	    $catename       = isset($value['orderInfo']['catename'])        ? $value['orderInfo']['catename']:'';         //状态
        	    
        	    $tracknumber    = isset($value['orderInfo']['trackNumber'])    ? $value['orderInfo']['trackNumber']:'';//跟踪号
        	    
        	    $orderid        = isset($value['orderInfo']['id'])         ? $value['orderInfo']['id']:'';                    //订单号
        	    $carrier        = isset($value['orderInfo']['carrierName'])    ? $value['orderInfo']['carrierName']:'';     //运输方式
...
EOF;
        	    ...
        	    $skurow         = '';
        	    if (isset($value['skuList'])) {
        	        foreach ($value['skuList'] as $skuitem){
        	            switch(strtolower($skuitem['reviews'])){
        	            	case 1:
        	            ...
        	            if (empty($listInTwomonth) || (time() - $value['orderInfo']['ordersTime']) < 5184000 ) {
        	                $listInTwomonth    .= <<<EOF
                        	<tr style="background-color:#ffffff">
                                <td>$orderid</td>
                                <td>$buyer_account</td>
                                <td>$skuitem[sku]</td>
                                <td><a href="http://cgi.ebay.com/ws/eBayISAPI.dll?ViewItem&item=$skuitem[itemId]" target="_blank">$skuitem[itemId]</a></td>
                                <td>$skuitem[amount]</td>
                                <td>$skuitem[itemPrice]</td>
                                <td>$money($couny)</td>
                                <td>$catename</td>
                                <td>$paytime</td>
                                <td>$shiptime</td>
...
EOF;
                        	...
                        	<tr style="background-color:#ffffff">
                                ...
                                <td>$buyer_account</td>
                                <td>$skuitem[sku]</td>
                                <td><a href="http://cgi.ebay.com/ws/eBayISAPI.dll?ViewItem&item=$skuitem[recordNumber]" target="_blank">$skuitem[recordNumber]</a></td>
                                <td>$skuitem[amount]</td>
                                <td>$skuitem[itemPrice]</td>
                                <td>$money($couny)</td>
                                <td>$catename</td>
