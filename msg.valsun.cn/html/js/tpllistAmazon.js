function deletetpl(id){
	...
	var result = alertify.confirm("确定删除吗?",
		function (e){
			if(e) {	//点击确定
				$.getJSON(
					"index.php?mod=amazonMessageTemplate&act=ajaxDelTemplate&tid="+id,
					function(data){
...
function deleteshiptpl(id){
	...
	var result = alertify.confirm("确定删除吗?",
		function (e){
			if(e) {	//点击确定
				$.getJSON(
					"index.php?mod=amazonMessageTemplate&act=deleteShipTpl&id="+id,
					function(data){
...
function setTpl(account, obj){
	...
	var result = alertify.confirm("确定修改吗?",
			function (e){
				if(e) {	//点击确定
					$.getJSON(
						"index.php?mod=amazonMessageTemplate&act=setAccountsTpl&id="+val+"&account="+account,
						function(data){
...
function delCsTpl(id){
	var result = alertify.confirm("确定删除吗?",
			function (e){
				if(e) {																//点击确定
					$.getJSON(
						"index.php?mod=amazonMessageTemplate&act=delCsTpl&tid="+id,
						function(data){
							if(data.code == 0){										//删除失败
