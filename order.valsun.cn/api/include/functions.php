 ...
 */
function vita_post_url_content($url, $vars=''){
	$ch	=	curl_init();
	curl_setopt($ch,CURLOPT_URL,$url);
...
//签名函数,POST传递时用的
function createSignP($paramArr, $app_secret='') {
	if($app_secret == ''){
...
//调用开放系统
function callOpenSystemPost($paramArr , $url=''){
	//global $url,$token; 
	if (empty($url)) {
		$url 	= C('OPEN_SYS_URL');
	} else {
	...
	//连接超时自动重试3次
	$cnt=0;	
	while($cnt < 3 && ($result=@vita_post_url_content($urls,$paramArr))===FALSE) $cnt++;
	return $result;
}
