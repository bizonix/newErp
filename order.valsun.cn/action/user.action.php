...
class UserAct{
	...
	*/	
	...
	public function act_userLogin(){
        ...
        $result			= UserModel::userLogin($username, $password/*, $companyId*/);
		$UserCompetenceAct = new UserCompetenceAct();
		$accountList = $UserCompetenceAct->act_showGlobalUser();
		$platformList = $UserCompetenceAct->act_showGlobalPlatform();
		...
		if(is_array($platformList)){
			$platformList = array_filter($platformList);
		}else{
			$platformList = array();	
		}
		$accountacc0 = '';
		...
		for($i=0;$i<count($accountList);$i++){
			$accountacc0 .= "da.accountId='".$accountList[$i]."' or ";
			$accountacc	.= "accountId='".$accountList[$i]."' or ";
		}
		$accountacc0    = substr($accountacc0,0,strlen($accountacc0)-3);
		$accountacc    = substr($accountacc,0,strlen($accountacc)-3);
		$_SESSION['platformList'] = $platformList;
		$_SESSION['accountList'] = $accountList;
		$_SESSION['accountacc'] = $accountacc;
		$_SESSION['accountacc0'] = $accountacc0;
		return $result;
    }
