...
class ExportsToXlsModel{
	...
	*/
	 ...
	 * 获取海外仓销售报表的数据
	 */
	public static function getEbayOversea($where) {
		self::$initDB();
		$sql = 'SELECT * FROM om_unshipped_order WHERE orderStatus = "709" AND combineOrder != 1 and (a.scantime >=$start and a.scantime <=$end) and ('.$where.')';
		echo $sql;exit;
		$query  = self::$dbConn->query($sql);
		if(!$query) {
			self::$errCode = '1804';
