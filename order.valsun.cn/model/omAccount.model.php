...
class omAccountModel{	
	...
	*/
	...
	/*
	 * 把id格式的json数据转换为名字格式的json数据
	 */
	public static function idTransferName($movefolders){
		!self::$dbConn ? self::initDB() : null;
		$movefolders = json_decode($movefolders,true);
		$movefolderNames = array();
		//获取所有文件夹名字，不用循环读取数据库
		$select = " statusName,statusCode ";
		$where  = " WHERE 1";
		$statusMenus  = StatusMenuModel::getStatusMenuListById($select, $where);
		foreach($movefolders as $key=>$movefolder){
			/*$ids = $movefolder;
			array_push($ids, $key);
			$ids_str = "'".implode("','",$movefolder)."'";
			$select = " SELECT statusName,statusCode ";
			$where  = " WHERE statusCode in({$id_str})";
			$statusMenus  = StatusMenuModel::getStatusMenuListById($select, $where);*/
			if(!empty($key)){
				foreach($movefolder as $statusCode){
					$movefolderNames[$statusMenus[$key]][] = $statusMenus[$statusCode];
				} 
			}
		}
		return json_encode($movefolderNames,JSON_UNESCAPED_UNICODE);
	}
}
?>	
