<?php
/**
 * 系统管理 systemManage.view.php
 * @author chenwei 2013.11.1
 */
class SystemManageView extends BaseView {
	...
	public function view_systemManageList() {
		...
		if(isset($_POST) && !empty($_POST)){			
			$keyWords	  = trim($_POST['keyWords']);//填写系统名称
			if(!empty($keyWords)){
		...
		$navlist = array (array ('url' => 'index.php?mod=nameSystem&act=nameSystemList','title' => '系统名称中心'),
						  array ('url' => 'index.php?mod=systemManage&act=systemManageList','title' => '系统名称管理'));
		$this->smarty->assign('navlist', $navlist);//二级导航
		$this->smarty->assign('toplevel', 0);//一级导航		
		$this->smarty->assign('systemListArr', $systemListArr);//显示数组
		//搜索选项值保留
