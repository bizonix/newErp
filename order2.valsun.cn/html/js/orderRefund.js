...
//初始化Dialog弹框 add by rdh 2013/09/16 paypal退款
	...
	form.dialog({
		...
		buttons : {
			'确定' : function(e) {
                ...
                orderobj.orderType = 1;//payapl退款处理
                ...
                alertify.confirm( message, function(e) {
                	if(e) {  //if clicking OK
                        $.ajax({
                        	type    :"POST",
                        	url     :"index.php?mod=orderManage&act=addRefundInfo",
                        	dataType:"json",
...
//初始化 手工退款   Dialog弹框 add by Herman.Xi 2014/01/07 modify by yxd 2014/08/22 
	...
	form.dialog({
		...
		buttons : {
			'确定' : function(e) {
                ...
                orderobj.platformId          = g_order_info.platformId;
                orderobj.currency            = g_order_info.currency;
                orderobj.accountId           = g_order_info.accountId;//sellerAccountId;
                orderobj.PayPalPaymentId     = g_order_info.PayPalPaymentId;
                orderobj.recordNumber        = g_order_info.recordNumber;
                orderobj.platformUsername    = g_order_info.platformUsername;
                orderobj.countryName         = g_order_info.countryName;
                	...
                	if(e) {  
                        $.ajax({
                        	...
                        	success :function(rtn){
								...
								$("#add_form_handapply").dialog('close');
                            }
                        });
                	}
                });
...
//申请 paypal退款 add by rdh 2013/09/16 modify by yxd 2014/08/22
function applyRefund() {
	...
	$.ajax({
		type    :"POST",
		url     :"index.php?mod=orderManage&act=applyRefund",
		dataType:"json",
		...
		success :function(rtn){
    		...
    		if(rtn.errCode==200){
               ...
               // $('#pop_platform').data(rtn.data.accountId);
                ...
                //alertify.alert(g_order_info.countryName);return false;
				if(orderStatus == 239 && orderType == 240){//已经发货
					initialPopWindow(1);
				}else{
					initialPopWindow(2);
				}
...
//手工退款 add by Herman.Xi 2014/01/06 modify by yxd 2014/08/13
function handRefund() {
	var checkBox = $("[name='ckb']:checked");
	if(checkBox.length == 0){
		alertify.alert('请选择要操作的项！');
