...
function getuserinfo(){
	...
	$.ajax({
		type	: "POST",
		dataType: "json",
		url		: 'index.php?mod=orderAdd&act=getBuyerinfo',
...
function change_platform(){
	...
	$.ajax({
			type	: "POST",
			dataType: "json",
			url		: 'index.php?mod=orderAdd&act=getAccountListByPlatform',
...
function fillsku(){
    ...
    //alert(sku_row);return;
    all_sku_arr = new Array();
    all_sku_arr = sku_row.split('\n');
    sku_arr = new Array();
    
    for (value in all_sku_arr){
       if($.trim(all_sku_arr[value])){
         sku_arr.push(all_sku_arr[value].split('\t'));
       }       
	}
    var len = sku_arr.length;
    ...
    if(f_sku_value==''){
    	for(var i=0;i<len;i++){
    		...
    		sku_obj.value = sku_arr[i][0];
    		var qty_obj = document.getElementById('tqty'+(now_tr_num-1));
    		qty_obj.value = sku_arr[i][1];
            var name_obj = document.getElementById('tname'+(now_tr_num-1));
    		name_obj.value = sku_arr[i][2];
    		now_tr_num++;
    	}
    }else{
    	for(var i=0;i<len;i++){
