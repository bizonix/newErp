...
$(function(){
	...
	$("#eidtorder_cka").click(function(){
		var ck    = $(this).attr('checked');
		if(ck){
			$("input[name='checkboxes_orderoptions[]']").prop('checked',true);
		}else{
			$("input[name='checkboxes_orderoptions[]']").each(function(){
	...
	$("#move_cka").click(function(){
		var ck    = $(this).attr('checked');
		
		if(ck){
			$("input[name='movein[]']").prop('checked',true);
		}else{
			$("input[name='movein[]']").each(function(){
				$(this).attr('checked',false);
			});
		}
	...
	$("#k_cka").click(function(){
		var ck    = $(this).attr('checked');
		
		if(ck){
			$("input[name='carrierListk[]']").prop('checked',true);
		}else{
			$("input[name='carrierListk[]']").each(function(){
				$(this).attr('checked',false);
			});
		}
	...
	$("#nk_cka").click(function(){
		var ck    = $(this).attr('checked');
		
		if(ck){
			$("input[name='carrierListnk[]']").prop('checked',true);
		}else{
			$("input[name='carrierListnk[]']").each(function(){
				$(this).attr('checked',false);
			});
		}
...
function showfolderCheckedAll(gid){
	var   ck    = $("#showfolder_cka"+gid).attr("checked");
	if(ck){
		$("input[gid='"+gid+"']").prop('checked',true);
	}else{
		$("input[gid='"+gid+"']").each(function(){
			$(this).attr('checked',false);
		});
	}
}

function movecka(mid,marr){
	$("input[name='movein[]']").each(function(){
		$(this).attr('checked',false);
	});
	var allarr= eval(marr);
	  for(var key in allarr){
		  if(mid==key){
			  for(var id in allarr[key]){
				  //alert(allarr[key][id]);
				  $("#in"+allarr[key][id]).attr('checked',true);
			  }
		  }
	  }
}
