...
class InterfacePurchaseModel extends InterfaceModel{
	 ...
	 * @author lzx
	 */
	public function check_sku($skuDetail, $num = 1){
		$sku = $skuDetail['sku'];
		...
		$goods_stock = $goodsCountInfo['data'];
		
		$everyday_sale = F('SkuDailyInfo')->getSkuAverageDailyCount($sku);
		
		//$everyday_sale = $sku_info['AverageDailyCount'];
		$takenum = ceil($everyday_sale*10);
		
		$actuallaygoods = $goods_stock;
		if($actuallaygoods == 0){
			$goods_bili = 0;
		}else{
    ...
    /**
	 * 批量查询多个sku的采购天数及预警天数
	 * @param string sku数组的Json字符串
	 * @return array 
	 * @author zqt
	 */
	public function getStatisticsInfo($skuArr){
