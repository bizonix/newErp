...
class Core {
	...
	private function __construct(){
		...
		include	WEB_PATH."lib/rabbitmq/rabbitmq.class.php";		//消息队列
		...
		include	WEB_PATH."lib/class.curl.php";		//memcache
		...
		if(C("DATAGATE") == "db"){
			...
			if($db	==	"mysql"){
				...
				$dbconn = $dbConn;
				$rmqObj = new RabbitMQClass($rmq_config['user'],$rmq_config['passwd'],$rmq_config['vhost'],$rmq_config['ip']);//队列对象
			}
			if($db	==	"mongodb"){
				//.......
			}
