...
function track_open(){
	...
	window.location.href = "index.php?mod=trackInquiry&act=track&carrier=" + carrier + "&tracknum=" + tracknum;
	return false;
}
	...
	$.post(url,data,function(rtn){
		if(rtn.errCode == 0) {
			...
			for (var i=0; i<obj.length; i++) {
				...
				if (i==(obj.length-1) && (ext['file_cz']!='' && typeof(ext['file_cz'])!='undefined')) {
					$("#pic1").attr('src',ext['file_cz']);
					$("#track_pic1").show();
				}
				if (i==(obj.length-1) && (ext['file_fh']!='' && typeof(ext['file_fh'])!='undefined')) {
				...
				if (i==(obj.length-1)) {
					...
					$("#consTime").html('Time consuming：<span>'+day+' days '+hour+' hours</span>');
				}				
            }			
...
function nextScreen(num,cid){
	...
	var objs = document.getElementById('tradings'+cid).getElementsByTagName("li");
	var nums = objs.length;
	for (var i=0; i<nums; i++) {
		if(i<num) {
			objs[i].className = "dis_none";
		} else {
			objs[i].className = "";
		}
	}
