...
// 			$spu = $defectiveProductsList[0]['spu'];
	...
	public function act_operatePendingProducts() { //QC对待定列表中进行修改图片，正常回测，待退回（全部，不分部分）
		...
		try {
			...
			$auditId   = isset($_GET['sysUserId']) ? post_check($_GET['sysUserId']) : '';
			$note 	   = isset($_GET['note']) ? post_check($_GET['note']) : '';
			...
			TransactionBaseModel :: begin();
		/*	$selectStatus = $pendingProductsList[0]['pendingStatus'];（暂时屏蔽）
...
// 			$spu = $pendingProductsList[0]['spu'];
			...
			elseif ($status == 2) { //正常回测
				$set = "SET pendingStatus='5',auditId='{$auditId}',lastModified='{$now}',backNote='{$note}' "; //直接进行逻辑删除此条待定信息
				$where = "WHERE id='$pendingId'";
				//echo $set;
				PendingProductsModel :: updatePendingProducts($set, $where); //先将该待定记录的相关字段修改
