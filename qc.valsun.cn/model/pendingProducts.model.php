...
class PendingProductsModel {
	...
	*/
	...
	//从qc_sample_info表中取出数据
	public static function getWhInfo($select, $where) {
		self :: initDB();
		$sql = $sql = "SELECT $select FROM qc_sample_info $where ";
		//echo $sql . '<br>';
		$query = self :: $dbConn->query($sql);
		if ($query) {
			$ret = self :: $dbConn->fetch_array_all($query);
	...
	public static function updateWhInfo($set, $where) {
		self :: initDB();
		$sql = "UPDATE qc_sample_info $set $where";
		//echo $sql . '<br>';
		if (!self :: $dbConn->query($sql)) {
			self :: $errCode = "1701";
	...
	public static function addgetWhInfo($set) {
		self :: initDB();
		$sql = "INSERT INTO qc_sample_info $set";
		//echo $sql . '<br>';
		if (!self :: $dbConn->query($sql)) {
			self :: $errCode = "1801";
			self :: $errMsg = "qc_sample_info";
			return false;
		} else {
			return self :: $dbConn->affected_rows();
		}
